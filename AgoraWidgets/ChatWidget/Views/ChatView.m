...
- (void)setupSubViews
{
    ...
    self.nilMsgLable.text = [@"fcr_hyphenate_im_no_message" widget_localized];
...
- (UIMenuItem *)recallMenuItem
{
    if (_recallMenuItem == nil) {
        _recallMenuItem = [[UIMenuItem alloc] initWithTitle:[@"fcr_hyphenate_im_recall" widget_localized] action:@selector(recallMenuItemAction:)];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([obj isKindOfClass:[NSString class]]) {
        ...
        if (model.type == EMMessageTypeExtRecall) {
            cellString = [@"fcr_hyphenate_im_recall_a_message" widget_localized];
        }
        if (model.emModel.body.type == EMMessageBodyTypeCmd) {
            ...
            if([action isEqualToString:@"DEL"]) {
                cellString = [@"fcr_hyphenate_im_teacher_remove_message" widget_localized];
            }
            if([action isEqualToString:@"setAllMute"]) {
                cellString = [@"fcr_hyphenate_im_teacher_mute_all" widget_localized];
            }
            if([action isEqualToString:@"removeAllMute"]) {
                cellString = [@"fcr_hyphenate_im_teacher_unmute_all" widget_localized];
            }
            if([action isEqualToString:@"mute"] || [action isEqualToString:@"unmute"]) {
                ...
                if(muteMember.length > 0 && [[EMClient sharedClient].currentUsername isEqualToString:muteMember]) {
                    ...
                    if([action isEqualToString:@"mute"]) {
                        NSString *str = [@"fcr_hyphenate_im_muted_by_teacher" widget_localized];
                        cellString = [str stringByReplacingOccurrencesOfString:Ag_localized_replacing
                                                                    withString:teacherNickName];
                    }else{
                        NSString *str = [@"fcr_hyphenate_im_unmuted_by_teacher" widget_localized];
                        cellString = [str stringByReplacingOccurrencesOfString:Ag_localized_replacing
                                                                    withString:teacherNickName];
                    }
...
- (void)recallMenuItemAction:(UIMenuItem *)aItem
{
    ...
    [[EMClient sharedClient].chatManager recallMessageWithMessageId:model.emModel.messageId completion:^(EMError *aError) {
        if (!aError) {
            EMTextMessageBody *body = [[EMTextMessageBody alloc] initWithText:[@"fcr_hyphenate_im_recall_a_message" widget_localized]];
